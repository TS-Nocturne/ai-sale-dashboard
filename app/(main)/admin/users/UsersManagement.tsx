...
const ALL_ROLES = ["user", "employee", "manager", "admin"] as const
...
export default function UsersManagement() {
    ...
    const [formBanDuration, setFormBanDuration] = useState<number>(0)
    ...
    // ดึงข้อมูลผู้ใช้ใหม่เมื่อเปลี่ยนหน้า/คำค้นหา (fetchUsers ตั้ง loading เองเป็นปกติของ data fetch)
    ...
    const resetForm = () => {
        ...
        setFormRoles(["user"]); setFormBanReason(""); setFormBanDuration(0)
        setActionError(""); setActionSuccess("")
    }

    const openModal = (type: ModalType, user?: User) => {
        resetForm(); setModal(type)
        if (user) {
            setSelectedUser(user)
            setFormRoles(user.role.split(",").map((r) => r.trim()) as RoleType[])
            if (type === "editUser") setFormName(user.name)
        }
    }

    const closeModal = () => { setModal(null); setSelectedUser(null); resetForm() }
    ...
    const handleCreateUser = async () => {
        ...
        try {
            ...
            closeModal(); showSuccess("สร้างผู้ใช้ใหม่สำเร็จ"); fetchUsers()
        } catch (err: unknown) {
    ...
    const handleUpdateUser = async () => {
        ...
        setActionLoading(true); setActionError("")
        try {
            const res = await authClient.admin.updateUser({ userId: selectedUser.id, data: { name: formName } })
            if (res.error) throw new Error(res.error.message ?? "Update failed")
            closeModal(); showSuccess("อัปเดตข้อมูลผู้ใช้สำเร็จ"); fetchUsers()
        } catch (err: unknown) {
            setActionError(err instanceof Error ? err.message : "เกิดข้อผิดพลาด")
        } finally { setActionLoading(false) }
    }
    ...
    const handleSetRole = async () => {
        ...
        try {
            ...
            closeModal(); showSuccess(`เปลี่ยน role เป็น "${formRoles.join(", ")}" สำเร็จ`); fetchUsers()
        } catch (err: unknown) {
            setActionError(err instanceof Error ? err.message : "เกิดข้อผิดพลาด")
        } finally { setActionLoading(false) }
    }
    ...
    const handleBanUser = async () => {
        ...
        try {
            ...
            closeModal(); showSuccess("แบนผู้ใช้สำเร็จ"); fetchUsers()
        } catch (err: unknown) {
            setActionError(err instanceof Error ? err.message : "เกิดข้อผิดพลาด")
        } finally { setActionLoading(false) }
    }
    ...
    const handleUnbanUser = async (user: User) => {
        try {
            ...
            showSuccess("ปลดแบนผู้ใช้สำเร็จ"); fetchUsers()
        } catch { console.error("Unban failed") }
    }
    ...
    const handleSetPassword = async () => {
        ...
        setActionLoading(true); setActionError("")
        try {
            const res = await authClient.admin.setUserPassword({ userId: selectedUser.id, newPassword: formPassword })
            if (res.error) throw new Error(res.error.message ?? "Set password failed")
            closeModal(); showSuccess("เปลี่ยนรหัสผ่านสำเร็จ")
        } catch (err: unknown) {
            setActionError(err instanceof Error ? err.message : "เกิดข้อผิดพลาด")
        } finally { setActionLoading(false) }
    }
    ...
    const handleDeleteUser = async () => {
        ...
        try {
            ...
            closeModal(); showSuccess("ลบผู้ใช้สำเร็จ"); fetchUsers()
        } catch (err: unknown) {
            setActionError(err instanceof Error ? err.message : "เกิดข้อผิดพลาด")
        } finally { setActionLoading(false) }
    }
    ...
    const handleImpersonate = async (user: User) => {
        ...
        } catch { console.error("Impersonate failed") }
    }
    ...
    const handleStopImpersonating = async () => {
        ...
        } catch { console.error("Stop impersonating failed") }
    }
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">ทั้งหมด {totalUsers} คน</p>
                </div>
                <button
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-border bg-muted/50">
                                <th className="px-4 py-3 text-left font-medium text-muted-foreground">ผู้ใช้</th>
                                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Role</th>
                                <th className="px-4 py-3 text-left font-medium text-muted-foreground">สถานะ</th>
                                <th className="px-4 py-3 text-left font-medium text-muted-foreground">สมัครเมื่อ</th>
                                    ...
                                    >
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-3">
                                                {user.image ? (
                                                    // อวตารมาจากโดเมน OAuth ภายนอกที่หลากหลาย จึงใช้ <img> แทน next/image
            ...
            {/* Dropdown (rendered outside table to avoid overflow clipping) */}
            {openDropdown && (() => {
                ...
                return (
                    <>
                        ...
                        <div
                            className="fixed z-50 w-48 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-1 shadow-lg"
                            style={{ top: dropdownPos.top, left: dropdownPos.left }}
                        >
                            <DropdownItem icon={Pencil} label="แก้ไขข้อมูล" onClick={() => { setOpenDropdown(null); openModal("editUser", user) }} />
                            <DropdownItem icon={Shield} label="เปลี่ยน Role" onClick={() => { setOpenDropdown(null); openModal("setRole", user) }} />
                            <DropdownItem icon={Key} label="เปลี่ยนรหัสผ่าน" onClick={() => { setOpenDropdown(null); openModal("setPassword", user) }} />
                            {user.banned ? (
                                <DropdownItem icon={UserCheck} label="ปลดแบน" onClick={() => { setOpenDropdown(null); handleUnbanUser(user) }} />
                            ) : (
                                <DropdownItem icon={Ban} label="แบนผู้ใช้" className="text-amber-600 dark:text-amber-400" onClick={() => { setOpenDropdown(null); openModal("ban", user) }} />
                            )}
                            <DropdownItem icon={UserX} label="Impersonate" onClick={() => { setOpenDropdown(null); handleImpersonate(user) }} />
                            <div className="my-1 border-t border-border" />
                            <DropdownItem icon={Trash2} label="ลบผู้ใช้" className="text-red-600 dark:text-red-400" onClick={() => { setOpenDropdown(null); openModal("delete", user) }} />
                        </div>
                    </>
                )
            })()}
            ...
            {modal === "create" && (
                <Modal title="สร้างผู้ใช้ใหม่" onClose={closeModal}>
                    <div className="space-y-4">
                        <FormField label="ชื่อ" value={formName} onChange={setFormName} placeholder="ชื่อผู้ใช้" />
                        <FormField label="อีเมล" type="email" value={formEmail} onChange={setFormEmail} placeholder="email@example.com" />
                        <FormField label="รหัสผ่าน" type="password" value={formPassword} onChange={setFormPassword} placeholder="อย่างน้อย 8 ตัวอักษร" />
                        <div>
                            <label className="mb-1.5 block text-sm font-medium text-foreground">Role (เลือกได้หลาย role)</label>
                            <RoleCheckboxes selectedRoles={formRoles} onToggle={toggleRole} />
                        </div>
                        {actionError && <ErrorMsg msg={actionError} />}
                        <ModalActions loading={actionLoading} confirmLabel="สร้างผู้ใช้" onCancel={closeModal} onConfirm={handleCreateUser} />
                    </div>
                </Modal>
            )}

            {modal === "editUser" && selectedUser && (
                <Modal title={`แก้ไขข้อมูล — ${selectedUser.name}`} onClose={closeModal}>
                    <div className="space-y-4">
                        <div className="text-sm text-muted-foreground">อีเมล: <strong>{selectedUser.email}</strong></div>
                        <FormField label="ชื่อ" value={formName} onChange={setFormName} placeholder="ชื่อผู้ใช้" />
                        {actionError && <ErrorMsg msg={actionError} />}
                        <ModalActions loading={actionLoading} confirmLabel="บันทึก" onCancel={closeModal} onConfirm={handleUpdateUser} />
                    </div>
                </Modal>
            )}

            {modal === "setRole" && selectedUser && (
                <Modal title={`เปลี่ยน Role — ${selectedUser.name}`} onClose={closeModal}>
                    <div className="space-y-4">
                        ...
                        {actionError && <ErrorMsg msg={actionError} />}
                        <ModalActions loading={actionLoading} confirmLabel="บันทึก" onCancel={closeModal} onConfirm={handleSetRole} />
                    </div>
                </Modal>
            )}
            ...
            {modal === "ban" && selectedUser && (
                <Modal title={`แบนผู้ใช้ — ${selectedUser.name}`} onClose={closeModal}>
                    <div className="space-y-4">
                        <p className="text-sm text-muted-foreground">ผู้ใช้ที่ถูกแบนจะไม่สามารถเข้าสู่ระบบได้</p>
                        <FormField label="เหตุผล (ไม่บังคับ)" value={formBanReason} onChange={setFormBanReason} placeholder="เช่น Spam, ละเมิดกฎ" />
                        <div>
                            <label className="block text-sm font-medium mb-1.5 text-foreground">ระยะเวลาแบน</label>
                            <select
                        ...
                        {actionError && <ErrorMsg msg={actionError} />}
                        <ModalActions loading={actionLoading} confirmLabel="แบนผู้ใช้" variant="destructive" onCancel={closeModal} onConfirm={handleBanUser} />
                    </div>
                </Modal>
            )}

            {modal === "setPassword" && selectedUser && (
                <Modal title={`เปลี่ยนรหัสผ่าน — ${selectedUser.name}`} onClose={closeModal}>
                    <div className="space-y-4">
                        <FormField label="รหัสผ่านใหม่" type="password" value={formPassword} onChange={setFormPassword} placeholder="อย่างน้อย 8 ตัวอักษร" />
                        {actionError && <ErrorMsg msg={actionError} />}
                        <ModalActions loading={actionLoading} confirmLabel="เปลี่ยนรหัสผ่าน" onCancel={closeModal} onConfirm={handleSetPassword} />
                    </div>
                </Modal>
            )}
                    ...
                    <div className="space-y-4">
                        <div className="rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-800 dark:bg-red-900/20">
                            <p className="text-sm text-red-700 dark:text-red-400">
                                ⚠️ การลบผู้ใช้จะไม่สามารถกู้คืนได้ ข้อมูลทั้งหมดจะถูกลบถาวร
                            </p>
                        </div>
                        ...
                        {actionError && <ErrorMsg msg={actionError} />}
                        <ModalActions loading={actionLoading} confirmLabel="ลบผู้ใช้" variant="destructive" onCancel={closeModal} onConfirm={handleDeleteUser} />
                    </div>
                </Modal>
            )}
        </div>
...
const ROLE_BADGE_ICONS: Record<string, React.ReactNode> = {
    ...
    employee: <Headset className="h-3 w-3" />,
}
            ...
            {roles.map((r) => (
                <span
                    ...
                    className={cn(
                        ...
                        ROLE_BADGE_COLORS[r] ?? "bg-gray-100 text-gray-700"
                    )}
                >
                    {ROLE_BADGE_ICONS[r]}
                    {r}
    ...
    return (
        <div className="flex justify-end gap-2 pt-2">
            <button
                onClick={onCancel}
                disabled={loading}
                className="h-9 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-4 text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors cursor-pointer disabled:opacity-50"
            >
                ยกเลิก
            </button>
            <button
